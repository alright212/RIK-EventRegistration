...
namespace EventRegistration.Tests
{
    public class ParticipantServiceTests
    {
        ...
        public async Task AddIndividualParticipantAsync_ShouldThrowException_WhenEventNotFound()
        {
            ...
            // Act & Assert
            var exception = await Assert.ThrowsAsync<ArgumentException>(() => _service.AddIndividualParticipantAsync(dto));
        ...
        public async Task AddIndividualParticipantAsync_ShouldThrowException_WhenAlreadyRegistered()
        {
            ...
            var participantId = 1;
            var dto = new AddIndividualParticipantDto { EventId = eventId, PersonalIdCode = "39001011234" };
            var existingEvent = new Event("Test Event", DateTime.UtcNow.AddDays(1), "Test Location", "");
            var existingParticipant = new IndividualParticipant("Jane", "Doe", dto.PersonalIdCode);
            
            // Manually set the ID since the setter is private and DB would normally assign it
            typeof(Participant).GetProperty(nameof(Participant.Id))?.SetValue(existingParticipant, participantId);


            _mockEventRepo.Setup(repo => repo.GetByIdAsync(eventId)).ReturnsAsync(existingEvent);
            _mockPaymentMethodRepo.Setup(repo => repo.GetByIdAsync(dto.PaymentMethodId)).ReturnsAsync(new PaymentMethod("Cash"));
            _mockParticipantRepo.Setup(repo => repo.GetIndividualByPersonalIdCodeAsync(dto.PersonalIdCode)).ReturnsAsync(existingParticipant);
            _mockEventParticipantRepo.Setup(repo => repo.GetByEventAndParticipantAsync(eventId, existingParticipant.Id)).ReturnsAsync(new EventParticipant()); // Simulate existing registration

            // Act & Assert
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => _service.AddIndividualParticipantAsync(dto));
        ...
        public async Task AddIndividualParticipantAsync_ShouldAddParticipant_WhenNotAlreadyRegistered()
        {
            ...
            _mockPaymentMethodRepo.Setup(repo => repo.GetByIdAsync(dto.PaymentMethodId)).ReturnsAsync(new PaymentMethod("Cash"));
            _mockParticipantRepo.Setup(repo => repo.GetIndividualByPersonalIdCodeAsync(dto.PersonalIdCode)).ReturnsAsync((IndividualParticipant)null); // Not an existing participant
            _mockEventParticipantRepo.Setup(repo => repo.AddAsync(It.IsAny<EventParticipant>())).Returns(Task.CompletedTask);

            // Act
            await _service.AddIndividualParticipantAsync(dto);

            // Assert
            _mockParticipantRepo.Verify(repo => repo.AddAsync(It.IsAny<IndividualParticipant>()), Times.Once);
            _mockEventParticipantRepo.Verify(repo => repo.AddAsync(It.IsAny<EventParticipant>()), Times.Once);
        }
        ...
        public async Task DeleteParticipantAsync_ShouldThrowException_WhenRegistrationNotFound()
        {
            ...
            _mockEventParticipantRepo.Setup(repo => repo.GetByEventAndParticipantAsync(eventId, participantId)).ReturnsAsync((EventParticipant)null);

            // Act & Assert
            var exception = await Assert.ThrowsAsync<ArgumentException>(() => _service.DeleteParticipantAsync(participantId, eventId));
        ...
        public async Task DeleteParticipantAsync_ShouldDeleteRegistration_WhenFound()
        {
            ...
            _mockEventParticipantRepo.Setup(repo => repo.GetByEventAndParticipantAsync(eventId, participantId)).ReturnsAsync(eventParticipant);
            _mockEventParticipantRepo.Setup(repo => repo.DeleteAsync(eventParticipant)).Returns(Task.CompletedTask);

            // Act
            await _service.DeleteParticipantAsync(participantId, eventId);

            // Assert
            _mockEventParticipantRepo.Verify(repo => repo.DeleteAsync(eventParticipant), Times.Once);
        }
